...
$EmailSigGroup = Get-MsolGroup -all | where-object { $_.DisplayName -eq "LSI Signature Injection - CAG Template"}
$EmailSigGroupUK = Get-MsolGroup -all | where-object { $_.DisplayName -eq "LSI Signature Injection - CAG Template UK"}

$MailBoxMessageConfigurationsWithoutSignature = Get-MsolGroupMember -GroupObjectId $EmailSigGroup.ObjectId
               ...
               <TD>
                  <TABLE id=emailsignatureDetails border=0 cellSpacing=0 cellPadding=1>
                       <TBODY>
                           ...
                           <TR>
                               <TD style=`"TEXT-ALIGN:  Left; FONT-SIZE: 13px;FONT-FAMILY: Arial, Helvetica; COLOR: #124C86`">Capital Assets Group</TD>
                           </TR>
                           <TR>
